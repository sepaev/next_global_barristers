...
export default async function sendMailApi(req, res) {
  ...
  const mailText =
    `Привет, Алексей. ${dateString} новый клиент: cтрана - [${user.country}], ` +
  ...
  const mailHtml =
    '<div style="font-family: monospace;"><h1 style="color: #323a43;">Привет, Алексей.</h1>' +
  ...
  const textTelegram =
    ...
    `%0A Клиент оставил заявку обратного звонка со странички - «${user.siteUrl}»`
  ...
  try {
    ...
    // console.log('telegramResult', telegramResult)
    if (telegramResult.ok) {
      console.log('telegram not send')
    } else {
      console.log('telegram not send')
    }
  } catch (error) {
